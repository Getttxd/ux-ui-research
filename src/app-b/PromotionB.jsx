...
export default function PromotionB({ 
      ...
      <div ref={contentRef} className="flex-1 overflow-y-auto pb-4 scrollbar-none">
        ...
        <div className="px-4 divide-y divide-slate-100">
          {promotions.map((promo, index) => {
            ...
            const isBottomPromo = index === promotions.length - 1
            const isDimmed = isBottomPromo || (selected.length > 0 && !isChecked)
            return (
              <div
                key={promo.id}
                onClick={() => {
                  if (isBottomPromo) return
                  handleSelectPromo(promo.id)
                }}
                className={`py-3.5 flex items-center justify-between rounded-xl px-1.5 transition-all duration-200 ${
                  isBottomPromo 
              ...
              >
                ...
                {/* Checkbox (bottom promo does not have checkbox) */}
                {!isBottomPromo && (
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 transition-all ${
                    isChecked 
                      ? 'bg-[#22a447] text-white shadow-2xs' 
                      : 'border-2 border-slate-400/90 bg-white'
                  }`}>
                    {isChecked && (
